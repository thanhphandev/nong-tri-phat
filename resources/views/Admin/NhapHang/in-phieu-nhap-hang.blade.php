...
@section('content')
            ...
            @foreach($nh['hanghoa'] as $key => $hh)
                <tr>
                    ...
                    <td class="text-center">
                        {{ !empty($hh['ngay_het_han']) ? App\Http\Controllers\ObjectController::getDate($hh['ngay_het_han'], "d/m/Y") : '-' }}
                    </td>
    ...
    <div class="summary-wrapper">
        ...
        @endphp
        <div class="bank-info"
            style="display: table-cell; width: 42%; text-align: left; vertical-align: top; border-right: 1px solid #eee; padding-right: 3mm;">
            @if(env('BANK_STK') && $tong_phai_tra_qr > 0)
                ...
                <div style="display: flex; align-items: center;">
                    <img src="https://img.vietqr.io/image/{{ env('BANK_ID') }}-{{ env('BANK_STK') }}-compact.png?amount={{ max(0, $tong_phai_tra_qr) }}&addInfo={{ $nh['ma_nhap_hang'] }}"
                        style="width: 25mm; border: 1px solid #f0f0f0; padding: 1px;">
        ...
        <table class="summary-table" style="width: 58%; padding-left: 3mm;">
            <tr>
                <td class="summary-label">GIÁ TRỊ PHIẾU NHẬP:</td>
                <td class="summary-value text-bold" style="font-size: 11pt;">
                    {{ number_format($gia_tri_lo_nay, 0, ",", ".") }}</td>
            </tr>
                ...
                if (isset($lich_su_thanh_toan)) {
                    foreach ($lich_su_thanh_toan as $ls) {
                        if (!isset($ls['id_trahangncc'])) {
                            $tong_da_tt_lo_nay += (float) $ls['tong_thanh_tien'];
                        }
                    }
                } else {
                    $tong_da_tt_lo_nay = (float) $da_thanh_toan_lo_nay;
                }
            @endphp

            @if($tong_da_tt_lo_nay > 0)
                <tr>
                    <td class="summary-label">Đã thanh toán (phiếu này):</td>
                    <td class="summary-value">- {{ number_format($tong_da_tt_lo_nay, 0, ",", ".") }}</td>
                </tr>
            @endif

            @if(isset($gia_tri_tra_hang_lo_nay) && $gia_tri_tra_hang_lo_nay > 0)
                <tr>
                    <td class="summary-label" style="color: #e67e22;">Tiền trả hàng (phiếu này):</td>
                    <td class="summary-value" style="color: #e67e22;">-
                        {{ number_format($gia_tri_tra_hang_lo_nay, 0, ",", ".") }}</td>
                </tr>
            @endif

            <tr style="border-top: 1px solid #ddd;">
                <td class="summary-label" style="font-style: italic;">Còn nợ phiếu này:</td>
                <td class="summary-value" style="font-style: italic;">{{ number_format($tong_no_moi, 0, ",", ".") }}</td>
            </tr>
            ...
            @if(isset($cong_no_ton_ncc) && $cong_no_ton_ncc != 0)
                <tr>
                    ...
                    <td class="summary-value">
                        {{ $cong_no_ton_ncc > 0 ? '+' : '' }}{{ number_format($cong_no_ton_ncc, 0, ",", ".") }}</td>
                </tr>
            @endif

            <tr class="summary-total" style="border-top: 2px solid #333;">
                <td class="summary-label" style="font-size: 12pt;">TỔNG NỢ:</td>

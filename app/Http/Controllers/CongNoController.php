...
class CongNoController extends Controller
{
    ...
    function list(Request $request)
    {
        ...
        // Initialize variables to avoid compact undefined errors
        $transaction_history = collect();
        $don_no_list = [];
        $don_am_list = [];
        $congno_sum = 0;
        $thanhtoan_sum = 0;
        ...
        if ($id_khachhang) {
            ...
            // --- Lấy danh sách Đơn Hàng Còn Nợ / Dư Tiền ---
            ...
            $stats_map = [];
            if (count($dh_ids) > 0) {
                $raw_stats_orders = CongNo::raw(function ($collection) use ($dh_ids) {
                    return $collection->aggregate([
                        [
                            '$match' => [
            ...
            $don_no_list = [];
            $don_am_list = []; // Danh sách dư tiền (Khách thanh toán dư/trả hàng)
            foreach ($orders as $dh) {
                $stats = isset($stats_map[(string) $dh->_id]) ? $stats_map[(string) $dh->_id] : ['tong_no' => 0, 'tong_tra' => 0, 'con_no' => 0];
                $con_no = $stats['con_no'];
                $order_data = [
                    'id_don_hang' => (string) $dh->_id,
                    'ngay_ban' => $dh->ngay_ban,
                    'ma_don_hang' => $dh->ma_don_hang,
                    'tong_thanh_tien' => $stats['tong_no'],
                    'da_thanh_toan' => $stats['tong_tra'],
                    'con_no' => $con_no
                ];
    ...
    function thanh_toan(Request $request)
    {
        ...
        $id_donhang_linked = isset($data['id_donhang']) && !empty($data['id_donhang']) ? ObjectController::ObjectId($data['id_donhang']) : null;
        $order_linked = $id_donhang_linked ? DonHang::find($data['id_donhang']) : null;

        // Nếu loại là GHI NỢ THÊM (0), tạo 1 record có thể link đơn
        if ($loai_cong_no == 0) {
            $congno = new CongNo();
            ...
            $congno->id_donhang = $id_donhang_linked;
            $congno->ma_don_hang = $order_linked ? $order_linked->ma_don_hang : '';
            ...
            $querLog = array(
                'action' => 'Ghi nợ/Cân bằng KH [' . $kh['ho_ten'] . '] - ' . number_format($so_tien, 0, ',', '.') . ' VND',
                'id_collection' => $congno->_id,
                'collection' => 'cong_no',
                'data' => $data
            );
            LogController::addLog($querLog);
            Session::flash('msg', 'Ghi nợ/Hoàn trả thành công: ' . number_format($so_tien, 0, ',', '.') . ' VND');
            return redirect($data['url']);
        }
        ...
        $don_hang_list = DonHang::where('id_khachhang', $id_khachhang_obj)
            ->where('tinh_trang', '!=', 2) // Exclude cancelled
        ...
        if ($id_donhang_linked) {
            $don_spec = null;
            foreach ($don_con_no as $d) {
                if ((string)$d['id'] == (string)$data['id_donhang']) {
                    $don_spec = $d;
                    break;
                }
            }
            ...
            if ($don_spec) {
                ...
                $congno->loai_khach_hang = $kh['loai_khach_hang'];
                $congno->id_donhang = $id_donhang_linked;
                $congno->ma_don_hang = $don_spec['ma'];
        ...
        // ƯU TIÊN 2: Phân bổ FIFO cho các đơn còn lại
    ...
    public function exportPdf(Request $request)
    {
        ...
        $donHangIds = $phatSinh->pluck('id_donhang')->filter()->map(function ($id) {
            return (string) $id;
        })->unique()->toArray();
        $traHangIds = $phatSinh->pluck('id_trahangkhach')->filter()->map(function ($id) {
            return (string) $id;
        })->unique()->toArray();

        $donHangObjectIds = array_map(function ($id) {
            return ObjectController::ObjectId($id);
        }, $donHangIds);
        ...
        $dataDonHang = DonHang::whereIn('_id', $donHangObjectIds)->get()->keyBy(function ($i) {
            return (string) $i->_id;
        });
        $dataTraHang = TraHangKhach::whereIn('_id', $traHangObjectIds)->get()->keyBy(function ($i) {
            return (string) $i->_id;
        });
        ...
        $allUnits = \App\Models\DonViTinh::all()->keyBy(function ($i) {
            return (string) $i->_id;
        });

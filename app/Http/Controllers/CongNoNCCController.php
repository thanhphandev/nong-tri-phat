...
class CongNoNCCController extends Controller
{
    ...
    function list(Request $request)
    {
        ...
        $product_history = [];
        $don_no_list = [];
        $don_am_list = []; // Initialize to avoid compact error
        $congno_sum = 0;
        $thanhtoan_sum = 0;
        $start_date = null;
        $end_date = null;
        ...
        if ($id_nhacungcap) {
            ...
            $q_nh = NhapHang::where('id_nhacungcap', $id_nhacungcap_obj);
            ...
            // --- Lấy danh sách Phiếu Nhập Còn Nợ / Dư Tiền ---
            ...
            $stats_map = [];
            if (count($nh_ids) > 0) {
                $raw_stats_orders = CongNoNCC::raw(function ($collection) use ($nh_ids) {
                    return $collection->aggregate([
                        [
                            '$match' => [
                ...
                foreach ($raw_stats_orders as $s) {
                    $stats_map[(string) $s['_id']] = [
                        'tong_no' => $s['tong_no'],
                        'tong_tra' => $s['tong_tra'],
                        'con_no' => $s['tong_no'] - $s['tong_tra']
                    ];
                }
            }

            $don_no_list = [];
            $don_am_list = []; // Danh sách dư tiền (cần hoàn trả)
            foreach ($import_orders as $nh) {
                $stats = isset($stats_map[(string) $nh->_id]) ? $stats_map[(string) $nh->_id] : ['tong_no' => 0, 'tong_tra' => 0, 'con_no' => 0];
                $con_no = $stats['con_no'];
                $order_data = [
                    ...
                    'tong_thanh_tien' => $stats['tong_no'], // Tổng nợ (Đơn + Adjustment)
                    'da_thanh_toan' => $stats['tong_tra'], // Đã trả + Trả hàng
                    'con_no' => $con_no
                ];

                if ($con_no > 0) {
                    $don_no_list[] = $order_data;
                } elseif ($con_no < 0) {
                    $don_am_list[] = $order_data;
                }
            }
        }
    ...
    function thanh_toan(Request $request)
    {
        ...
        $id_nhaphang_linked = isset($data['id_nhaphang']) && !empty($data['id_nhaphang']) ? ObjectController::ObjectId($data['id_nhaphang']) : null;
        $order_linked = $id_nhaphang_linked ? NhapHang::find($data['id_nhaphang']) : null;

        // Nếu loại là GHI NỢ THÊM (0), tạo 1 record đơn giản nhưng có thể link đơn
        if ($loai_cong_no == 0) {
            ...
            $congno->id_nhaphang = $id_nhaphang_linked;
            $congno->ma_nhap_hang = $order_linked ? $order_linked->ma_nhap_hang : '';
            $congno->so_chung_tu = $order_linked ? ($order_linked->so_chung_tu ?? '') : '';
            ...
            $querLog = array(
                'action' => 'Ghi nợ/Hoàn tiền NCC [' . $ncc['ten'] . '] - ' . number_format($so_tien, 0, ',', '.') . ' VND',
                'id_collection' => $congno->_id,
                'collection' => 'cong_no_ncc',
                'data' => $data
            );
            LogController::addLog($querLog);
            Session::flash('msg', 'Ghi nợ/Cân bằng thành công: ' . number_format($so_tien, 0, ',', '.') . ' VND');
            return redirect($data['url']);
        }
        ...
        $nhap_hang_list = NhapHang::where('id_nhacungcap', $id_nhacungcap_obj)
        ...
        // ƯU TIÊN 1: Nếu có chỉ định đơn hàng cụ thể
        if ($id_nhaphang_linked) {
            $don_spec = null;
            foreach ($don_con_no as $d) {
                if ((string)$d['id'] == (string)$data['id_nhaphang']) {
                    $don_spec = $d;
                    break;
                }
            }
            ...
            if ($don_spec) {
                $so_tien_tra_don_nay = min($so_tien_con_lai, $don_spec['con_no']);
                ...
                $congno->email = $ncc['email'];
                $congno->id_nhaphang = $id_nhaphang_linked;
                $congno->ma_nhap_hang = $don_spec['ma'];
                $congno->so_chung_tu = $don_spec['so_chung_tu'];
                $congno->tong_thanh_tien = $so_tien_tra_don_nay;
                $congno->ngay_gio = ObjectController::setDate();
                $congno->loai_cong_no = 1;
                $congno->ghi_chu = ($data['ghi_chu'] ?? 'Thanh toán');
                $congno->id_user = ObjectController::ObjectId($id_user);
                $congno->save();

                $da_phan_bo[] = $don_spec['ma'] . ': ' . number_format($so_tien_tra_don_nay, 0, ',', '.');
                $so_tien_con_lai -= $so_tien_tra_don_nay;
            }
        }

        // ƯU TIÊN 2: Phổ bổ FIFO cho các đơn còn lại
    ...
    public function exportPdfNCC(Request $request)
    {
        ...
        $nhapHangIds = $phatSinh->pluck('id_nhaphang')->filter()->map(function ($id) {
            return (string) $id;
        })->unique()->toArray();
        $traHangIds = $phatSinh->pluck('id_trahangncc')->filter()->map(function ($id) {
            return (string) $id;
        })->unique()->toArray();

        $nhapHangObjectIds = array_map(function ($id) {
            return ObjectController::ObjectId($id);
        }, $nhapHangIds);
        $traHangObjectIds = array_map(function ($id) {
            return ObjectController::ObjectId($id);
        }, $traHangIds);

        $dataNhapHang = NhapHang::whereIn('_id', $nhapHangObjectIds)->get()->keyBy(function ($i) {
            return (string) $i->_id;
        });
        $dataTraHang = \App\Models\TraHangNCC::whereIn('_id', $traHangObjectIds)->get()->keyBy(function ($i) {
            return (string) $i->_id;
        });
        ...
        $units = \App\Models\DonViTinh::all()->keyBy(function ($i) {
            return (string) $i->_id;
        });
    ...
    public function exportExcelNCC(Request $request)
    {
        ...
        $phatSinhTrongKySorted = $phatSinh->sortBy(function ($item) {
            return $item->ngay_gio->toDateTime()->getTimestamp();
        });
